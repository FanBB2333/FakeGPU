...
def test_static_validation_cli_writes_checkable_static_only_report(tmp_path: Path) -> None:
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "scripts/validation/static_memory_validation.py",
            "--static-only",
            "--workload",
    ...
    checked = subprocess.run(
        [
            sys.executable,
            "scripts/validation/check_static_memory_validation.py",
            "--path",
            str(report_path),
            "--allow-static-only",
...
def test_static_validation_bundle_separates_peak_and_graph_consistency(tmp_path: Path) -> None:
    from scripts.validation.aggregate_static_memory_validations import (
        aggregate_reports,
        render_markdown,
    )
...
def test_measured_report_allows_zero_backend_resident_calibration(tmp_path: Path) -> None:
    from scripts.validation.check_static_memory_validation import main as check_report
