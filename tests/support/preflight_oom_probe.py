...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=["pass", "oom", "alloc"], default="pass")
    parser.add_argument("--elements", type=int, default=300_000_000)
    ...
    with fakegpu.stage("forward"):
        ...
        if ns.mode == "alloc":
            x = torch.empty((ns.elements,), device="cuda", dtype=torch.float32)
            print(f"peak={torch.cuda.max_memory_allocated()}")
            del x
            return 0
