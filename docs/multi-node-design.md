...
- rank join timeout
- collective timeout
- collective 参数不一致并传播持久 async error
- 通过全局 rank、communicator seqno 和 direct collective 类型确定性注入 rank 故障
- 存活 rank 通过 `ncclCommShrink(..., NCCL_SHRINK_ABORT)` 排除故障 rank，并在连续编号的子 communicator 上继续 collective
- worker 通过 `os._exit(86)` 真实退出后，由 collective 超时推断缺席 rank 并显式 shrink
- 固定 world size 的 `torchrun --max-restarts=1` worker-group 重启；各 agent 通过共享 store 对齐恢复代次
- cluster JSON/Markdown 报告记录故障与恢复事件

尚未实现的扩展 fault model：

- link slowdown
- packet drop 的高层等价模拟
- node unavailable 与动态成员调整
- heartbeat 检测
- world size 变化后的 elastic 训练恢复
- 指定 seqno 卡住

当前 rank 故障通过 `FAKEGPU_NCCL_FAULT_RANK`、
`FAKEGPU_NCCL_FAULT_SEQNO` 和 `FAKEGPU_NCCL_FAULT_OPERATION` 配置。
更复杂的链路和时间窗口故障仍建议采用独立 YAML：
