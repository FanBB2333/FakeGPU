...
下一阶段如果要支持“多节点”场景，重点不再只是伪造设备本身，而是要补齐“分布式语义”：
...
本设计文档的目标，是为未来的多节点模拟能力提供一份可分阶段落地的技术方案。
...
- Phase 1 / Phase 2：只承诺 `disabled`、`simulate`
- `proxy`：实验性，至少放到真实 GPU 混合运行之后
- `passthrough`：更偏观测/对比用途，不应阻塞前两阶段实现
...
- 文档中不再把“`ncclBarrier`”当作第一阶段必需 API，因为它不应成为设计的中心依赖
...
- Phase 1：先不做 chunking，只限制 tensor 大小并给出清晰报错
- Phase 2：再补 chunked staging
- Phase 3：再考虑 streaming allgather / reduce-scatter 优化
...
- Phase 1 门槛：2 rank / 4 rank 单机通过
- Phase 2 门槛：单机模拟 2 节点 x 4 rank 通过
- Phase 3 门槛：拓扑参数变化能反映到报告结果
...
- `test/test_ddp_simple.py` 可以演化成多进程版本并稳定运行

### Phase 3：拓扑与带宽模型

目标：

- 加入 node / link / oversubscription 配置
- 模拟 intra-node 与 inter-node 差异
- 输出链路级统计

验收：

- 相同 workload 在不同 fabric 配置下，报告中通信耗时明显不同且可解释
...
- `hybrid + simulate`
- GPU buffer host staging
- 与现有 hybrid OOM / report 逻辑接通
- 评估是否值得引入 `proxy`

验收：

- 单机真实算子 + 虚拟跨节点 collective 可以稳定运行

### Phase 5：真实 NCCL 代理与多机扩展

目标：

- `proxy` / `passthrough`
- 真正支持远端 coordinator
- 为 parity 与 shadow-mode 分析提供基础

验收：

- 在有 GPU + 有 NCCL 的环境里，可比较真实通信与虚拟模型的偏差
...
- Milestone D：Step 16 ~ Step 18
  - 目标：把拓扑、时间模型、大 tensor 支持做完整
- Milestone E：Step 19 ~ Step 21
  - 目标：进入混合运行、真实 NCCL 代理和多机扩展
