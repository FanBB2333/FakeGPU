...
校准套件会写出 `build/rtx3090ti_calibration/calibration_rtx3090ti.json` 和 `.md`。如果当前机器没有 CUDA 可见的 RTX 3090 Ti，它会写出明确 skip 原因，不会静默通过。套件包含 tensor、MLP、Tiny Transformer、Hugging Face tiny GPT-2 和 PEFT LoRA tiny GPT-2 workload。报告会包含峰值误差、calibration factor，以及真实 CUDA 和 fakecuda 最大 timeline gap。校准误差只说明当前实现和这张 3090 Ti 上的小型 workload 表现，不代表 A100/H100 的可运行性、数值等价或性能。
