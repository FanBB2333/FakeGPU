...
- 梯度累积中途的 elastic DDP 恢复：AdamW 一阶/二阶矩、StepLR、完整 rank-state bundle、rank-local RNG、`DistributedSampler` cursor 与多 worker DataLoader 确定性重放
...
独立的参数化 DataLoader 重放矩阵不在上述默认集合中，因为每个运行时会创建
52 个 worker 进程。本地执行入口为：
...
| 场景 | World size | 每 rank worker | Prefetch | Epoch | Batch | 已提交 + 暂存批次 |
|---|---:|---:|---:|---:|---:|---:|
...
矩阵使用 shuffle `DistributedSampler`、常驻 spawn worker，并在数据集内分别
读取 PyTorch、Python 和 NumPy 随机数。检查内容包括 sampler 分区、不同 epoch
顺序、worker seed、已提交/暂存前缀重建，以及替换后的 worker PID。为兼容
PyTorch 2.8，验证器在保存前缀与 prefetch 诊断值后读取完当前 epoch，再关闭
worker group。它不会序列化活跃队列、正在处理的样本、IterableDataset 或外部
副作用。
...
中的全部 worker。每个 worker 会在 rendezvous store 中原子递增当前 rank 的
到达计数，以该计数区分共同代次，避免读取上一代留下的键值；报告仍保留不同
agent 可能不一致的本地 restart count。重启后的 group 必须得到梯度
`[1.5, 3.0]`、参数 `[0.85, -0.30]`，且两个 rank
...
训练状态恢复使用 `--case elastic-ddp-checkpoint`。首个 group 使用 momentum
为 `0.9` 的 SGD 完成一个 DDP step，每个 worker 通过原子替换写入一份主机本地
checkpoint，其中包含模型参数、optimizer 状态、world size 和
`completed_steps=1`。第二台主机以状态码 `86` 退出后，替换进程分别读取各自的
本地文件，对比两个 rank 恢复的张量状态，再执行第 2 步。恢复时的参数和
...
microstep 中的第一个。每个 rank 使用两个常驻 spawn worker 和
`prefetch_factor=2` 的带种子 `DataLoader`，数据集会在 worker 内调用
`torch.rand`。worker 退出前，所有 rank 会汇总各自尚未归约的梯度、CPU
generator RNG、`DistributedSampler` 的 epoch/cursor、DataLoader 构造参数、
三个已用于训练的批次，以及一个由应用层取出但尚未用于训练的暂存预取批次，
形成按原全局 rank 索引的 bundle。每台主机把完整 bundle 与模型、AdamW、
scheduler、optimizer 步数和 accumulation 状态写入一个原子 checkpoint。

维护中的测试会让当前 rank 0 读取原 rank 1 的状态，让当前 rank 1 读取原 rank 0
的状态。恢复时会重建 DataLoader，重放三个已提交批次；暂存样本必须仍为 8 和
7，worker ID 与 worker 生成的随机数也必须完全相同。重启后全部 DataLoader
worker PID 都必须变化。验证器还会检查恢复后的张量、下一次 rank-local RNG
采样、同步后的累积梯度与最终 AdamW 更新。另一个单元测试会交换两处 checkpoint
存储位置关联的 global rank，确认文件归属不会从重启后的 rank 推断。

该方案仍使用固定 world size。每台主机复制全部 rank-local 状态后，不再要求
同一主机重启前后保持相同 rank，也不需要共享文件系统；代价是每台主机保存
O(world-size) 的 rank 状态，并要求本地 checkpoint 仍然存在。恢复过程会重建
map-style DataLoader 并进行确定性重放，不会序列化仍在运行的多进程队列。
IterableDataset、外部副作用、非确定性 transform，以及 ZeRO/FSDP 分片 optimizer
状态不在当前恢复格式的覆盖范围内。该场景使用独立的
...
同一组主机还在 commit `f9c209c` 上通过了框架级 elastic DDP。RTX 3090 Ti
worker 以状态码 `86` 退出后，两台主机的 worker PID 都被 `torchrun` 替换。
两端本地 restart count 分别为 `1` 和 `0`，但都进入共享代次 1，建立第二个
communicator，并完成符合理论计算结果的 DDP 更新。Cluster 报告记录 4 次
All-Reduce、2 次 broadcast、2 次 All-Gather、224 字节节点对总量和 64 字节
节点对单次峰值。这些数据用于验证 coordinator 通信，不表示恢复吞吐率。

同一 commit 上连续两次执行 checkpoint 恢复，训练结果与通信统计完全一致。
两台主机都恢复了参数 `[0.85, -0.30]`、SGD momentum `[1.5, 3.0]` 和第 1 步
状态，继续执行后得到参数 `[0.565, -0.87]` 与 momentum `[2.85, 5.7]`。每次
实验创建 2 个 communicator，记录 4 次 All-Reduce、5 次 All-Gather、4 次
broadcast、512 字节节点对总量和 64 字节单次峰值。每个替换进程读取的文件
SHA-256 都与该主机保存时一致；不同 PyTorch 版本生成的文件可以不同，但恢复的
...
commit `ea390c0` 又完成了两次独立实验，并把每台主机的数据读取进程扩展为两个
常驻 DataLoader worker。经过 `0 → 1`、`1 → 0` 的 rank 映射后，当前 rank 0
和 1 分别精确重放暂存样本 8 和 7、worker ID 1，以及 worker 随机数
`0.47501373291015625` 和 `0.1462690830230713`；所有 DataLoader worker PID
均已变化。两次结果的最终参数和通信计数一致：4 次 All-Reduce、9 次
All-Gather、6 次 broadcast、25,960 字节节点对总量和 25,232 字节单次峰值。
基础重启、SGD checkpoint 与多 worker 训练状态恢复的组合实验创建 6 个
communicator，记录 26,680 字节节点对总量和 25,216 字节单次峰值。

2026-07-23，独立矩阵在本地以及 commit `5476f1e` 的两次物理控制器实验中
通过：

| 运行时 | 主机环境 | Python | PyTorch / CUDA | NumPy |
|---|---|---|---|---|
| macOS | 仅 CPU 控制端 | 3.11.9 | 2.9.1 / 无 CUDA | 2.2.6 |
| Linux | RTX PRO 5000，计算能力 12.0 | 3.10.15 | 2.8.0+cu128 / 12.8 | 1.26.4 |
| WSL2 | RTX 3090 Ti，计算能力 8.6 | 3.13.14 | 2.12.1+cu130 / 13.0 | 2.4.6 |
...
每个运行时包含 5 个场景、12 个 rank case 和 52 个 worker 进程；两次物理
实验的 worker PID 集合没有交集。该矩阵在 CPU 上执行 DataLoader 工作，证明
不同软件版本和不同 GPU 主机环境下的进程/RNG 重建一致；它本身不执行 GPU
kernel。

DataLoader-only 物理会话没有通信操作。Coordinator 仍会生成有效的 JSON 与
Markdown，保留配置中的四 rank、两节点拓扑；所有 collective 与 timeline
计数均为 0，RTX PRO 5000 ↔ RTX 3090 Ti 节点对也明确显示 0 字节。
...
accumulation 状态恢复可使用 `./ftest elastic_ddp_training_state`；独立 DataLoader
重放矩阵使用 `./ftest dataloader_replay`；完整故障测试使用
