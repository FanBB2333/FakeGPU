...
| `./ftest distributed_resilience` | 确定性 collective rank 故障、async error 传播、communicator shrink/恢复、TCP 参数不一致、缺少 rank 超时和报告时间线容量限制 |
...
| `python3 verification/run_physical_multihost.py ...` | 通过 SSH 重复执行两主机 Hybrid DDP/FSDP/FSDP2、混合精度、rank 故障恢复、参数不一致、超时、Git 版本和报告检查 |
...
- 注入故障与 communicator 恢复事件，包含全局 rank、操作、观测 ranks、尝试负载、排除项、存活项和恢复耗时
