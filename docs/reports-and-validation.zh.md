...
| `python3 verification/run_physical_multihost.py ...` | 通过 SSH 重复执行两主机 Hybrid DDP/FSDP、参数不一致、超时、Git 版本和报告检查 |
...
- 有大小限制的最近操作时间线，包含 communicator 全局 ranks、逻辑和 socket 负载、汇合时间、coordinator 执行时间及拓扑模型时间
...
17. 两台 GPU 主机同步到相同 commit 后，执行 `python3 verification/run_physical_multihost.py ...`。
18. 执行 `python test/run_error_simulation_suite.py`，检查错误模拟覆盖。
