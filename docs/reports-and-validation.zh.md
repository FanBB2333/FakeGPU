...
| `./ftest elastic_ddp` | 活跃 worker 退出、完整 `torchrun` worker group 替换、restart 代次同步、DDP 数值恢复、SGD checkpoint 恢复以及 AdamW accumulation 训练状态恢复 |
| `./ftest elastic_ddp_checkpoint` | 集中验证原子 checkpoint、训练步数、模型参数、SGD momentum 与 worker 替换后的继续更新 |
| `./ftest elastic_ddp_training_state` | 集中验证 AdamW 一阶/二阶矩、StepLR、rank-local RNG、optimizer step、待处理梯度与 accumulation 恢复 |
