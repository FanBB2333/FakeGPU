...
accumulation 状态版本会在两个梯度累积 microstep 中的第一个完成后注入故障。
替换进程恢复 AdamW 一阶/二阶矩、StepLR、已完成 optimizer 步数、待处理的
rank-local 梯度、rank-local RNG 和 `DistributedSampler` cursor，再完成需要
跨 rank 同步的第二个 microstep。每份主机 checkpoint 都包含所有原 rank 的
状态；维护中的测试会反转逻辑 rank 映射，确认恢复不依赖文件所有者原来的 rank。
本地使用 `./ftest elastic_ddp_training_state`，物理双机使用
`verification/run_physical_multihost.py --case elastic-ddp-training-state`。
