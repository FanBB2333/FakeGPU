...
from verification.run_qwen_fsdp_sft_memory import (  # noqa: E402
    _close_coordinator,
    _comparison,
    _find_free_port,
    _gib,
...
def compare_fsdp2_lora_reports(
    ...
    *,
    ...
    if static_report.get("training_method") != "lora":
        raise ValueError("the FSDP2 experiment requires a LoRA static report")
    ...
    for rank, report in enumerate(rank_reports):
        ...
        if report.get("fsdp2", {}).get("sharding_plan") != reference.get(
            "fsdp2", {}
        ).get("sharding_plan"):
            raise ValueError("rank reports contain different FSDP2 sharding plans")
        if report.get("loss_digest_sha256") != reference.get("loss_digest_sha256"):
    ...
    rank_results = []
    for rank, (report, projection) in enumerate(zip(rank_reports, projections)):
        comparisons = {
            phase: _comparison(
        ...
        rank_results.append(
            {
                ...
                "local_gradient_bytes": int(
                    report["parameters"]["local_gradient_storage_bytes"]
                ),
...
def render_markdown(report: Mapping[str, Any]) -> str:
    ...
    for rank in report["ranks"]:
        ...
        rows.append(
            f"| {rank['rank']} | "
            f"{_gib(comparisons['forward']['observed']):.3f} / "
            f"{_gib(comparisons['forward']['predicted']):.3f} / "
            f"{comparisons['forward']['absolute_error_percent']:.3f}% | "
            f"{_gib(comparisons['backward']['observed']):.3f} / "
            f"{_gib(comparisons['backward']['predicted']):.3f} / "
            f"{comparisons['backward']['absolute_error_percent']:.3f}% | "
    ...
    return "\n".join(
        [
            "# Qwen Hybrid FSDP2 LoRA SFT Memory Validation",
            "",
            ...
            f"- FSDP2 units: {report['sharding_plan']['unit_count']}; local "
            f"parameter storage {_gib(projection['local_parameter_bytes']):.3f} "
            f"GiB; local adapter gradients "
            f"{_gib(projection['local_gradient_bytes']):.3f} GiB",
            "",
            "Values are `observed / predicted / absolute error`.",
            "",
            "| Rank | Forward GiB | Backward GiB | Optimizer GiB | Overall GiB | Status |",
            "|---:|---:|---:|---:|---:|---|",
            *rows,
            "",
            "## Communication",
            ...
            f"- Reduce-scatter calls: "
            f"{collectives['reduce_scatter']['calls']}; bytes: "
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        command = [
            ...
            f"--lora-target-modules={args.lora_target_modules}",
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                f"FSDP2 LoRA workers exited with code {completed.returncode}"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "build" / "qwen-fsdp2-lora-sft-memory",
    )
    ...
    try:
        ...
        print(
            f"Qwen FSDP2 LoRA memory validation failed: "
