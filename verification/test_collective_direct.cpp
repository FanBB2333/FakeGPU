...
#include "../src/distributed/low_precision.hpp"
...
template <typename T>
std::vector<T> make_reduction_reference(
    int world_size,
    std::size_t count,
    ncclRedOp_t op) {
...
template <typename T>
std::vector<T> make_alltoall_reference(int world_size, std::size_t count, int rank) {
    const std::size_t total_count = count * static_cast<std::size_t>(world_size);
    std::vector<T> reference;
    reference.reserve(total_count);
    for (int peer = 0; peer < world_size; ++peer) {
        const std::vector<T> values = make_rank_values<T>(peer, total_count);
        const std::size_t offset = static_cast<std::size_t>(rank) * count;
        reference.insert(
            reference.end(),
...
using Decode16 = float (*)(std::uint16_t);
using Encode16 = std::uint16_t (*)(float);

void run_low_precision_allreduce_case(
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
        const bool grouped = iteration == 1;
        std::vector<std::vector<std::uint16_t>> send_buffers(
            world_size, std::vector<std::uint16_t>(count));
        std::vector<std::vector<std::uint16_t>> recv_buffers(
            world_size, std::vector<std::uint16_t>(count));
        std::vector<ncclResult_t> results(world_size, ncclInternalError);
        std::vector<std::thread> threads;

        for (int rank = 0; rank < world_size; ++rank) {
            for (std::size_t index = 0; index < count; ++index) {
                send_buffers[rank][index] = encode(
            ...
            threads.emplace_back([&, rank]() {
                if (grouped) {
                    require_result(
                        ncclGroupStart(),
                        ncclSuccess,
                        label + " ncclGroupStart failed");
                    const ncclResult_t inner = ncclAllReduce(
                        send_buffers[rank].data(),
                        recv_buffers[rank].data(),
                        count,
                        datatype,
                        op,
                        comms[rank],
        ...
        for (int rank = 0; rank < world_size; ++rank) {
            require_result(
                results[rank], ncclSuccess, label + " allreduce failed");
            for (std::size_t index = 0; index < count; ++index) {
                float expected =
                    3.0f + static_cast<float>(index) * 0.5f;
                if (op == ncclAvg) {
                    expected /= static_cast<float>(world_size);
                }
                require(
                    std::fabs(decode(recv_buffers[rank][index]) - expected) <=
                        tolerance,
                    label + " result mismatch at index " +
                        std::to_string(index));
            }
        }
    }
...
void run_allreduce_case(
    ...
    const std::vector<T> reference =
        make_reduction_reference<T>(world_size, count, op);
...
void run_alltoall_case(
    ...
    for (int iteration = 0; iteration < 3; ++iteration) {
        ...
        std::vector<std::vector<T>> recv_buffers(
            static_cast<std::size_t>(world_size),
            std::vector<T>(total_count, static_cast<T>(0)));
        std::vector<ncclResult_t> results(static_cast<std::size_t>(world_size), ncclInternalError);
        std::vector<std::thread> threads;
        ...
        for (int rank = 0; rank < world_size; ++rank) {
            threads.emplace_back([&, rank]() {
                if (grouped) {
                    require_result(ncclGroupStart(), ncclSuccess, label + " ncclGroupStart failed");
                    ncclResult_t inner = ncclAlltoAll(
                        send_buffers[static_cast<std::size_t>(rank)].data(),
                        recv_buffers[static_cast<std::size_t>(rank)].data(),
                        count,
                        datatype,
                ...
                results[static_cast<std::size_t>(rank)] = ncclAlltoAll(
                    send_buffers[static_cast<std::size_t>(rank)].data(),
                    recv_buffers[static_cast<std::size_t>(rank)].data(),
                    count,
                    datatype,
        ...
        for (int rank = 0; rank < world_size; ++rank) {
            require_result(results[static_cast<std::size_t>(rank)], ncclSuccess, label + " alltoall failed");
            assert_equal_vectors(
                recv_buffers[static_cast<std::size_t>(rank)],
                make_alltoall_reference<T>(world_size, count, rank),
                label + " alltoall result mismatch");
        }
    }
...
void run_broadcast_scenario() {
    ...
    run_broadcast_case<std::uint16_t>(
        2, 1, ncclFloat16, "2-rank float16 payload");
    run_broadcast_case<std::uint16_t>(
        2, 1, ncclBfloat16, "2-rank bfloat16 payload");
}
...
void run_allgather_scenario() {
    ...
    run_allgather_case<std::uint16_t>(
        2, ncclBfloat16, "2-rank bfloat16 payload");
}
...
void run_alltoall_scenario() {
    run_alltoall_case<float>(2, ncclFloat32, "2-rank float32");
    run_alltoall_case<float>(4, ncclFloat32, "4-rank float32");
    run_alltoall_case<std::int32_t>(4, ncclInt32, "4-rank int32");
}
...
void run_chunked_scenario() {
    ...
    run_alltoall_case<float>(4, ncclFloat32, "4-rank chunked alltoall", 16384);
}
