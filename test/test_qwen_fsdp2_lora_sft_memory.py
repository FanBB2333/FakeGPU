...
def _rank_report(rank: int, *, overall: int = 178) -> dict:
    return {
        "status": "success",
        "rank": rank,
        "world_size": 2,
        ...
        "memory_phases": {
            "forward_peak_bytes": 148,
            "backward_peak_bytes": 178,
            "graph_peak_bytes": 178,
...
def test_fsdp2_lora_comparison_accepts_matching_phase_predictions() -> None:
    ...
    assert report["projections"][0]["first_step_peak_bytes"] == 178
...
def test_fsdp2_lora_comparison_rejects_large_phase_error() -> None:
    report = compare_fsdp2_lora_reports(
        [_rank_report(0, overall=250), _rank_report(1, overall=250)],
        _static_report(),
        _cluster_report(),
