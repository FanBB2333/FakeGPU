...
| 验证层 | 已维护的检查 | 结果 |
|---|---|---|
| Python runtime、估算器、CLI、schema 和 README 契约 | 完整 `pytest` 测试集 | **163 个通过** |
...
| KV cache 分配 | Dynamic 增长、static 预留、2/4/8-bit quantized 存储、paged block 取整和 sliding-window 上限 | 公式、API、`--kv-cache-strategy` CLI 和 `tests/data/llm_validation.yaml` 矩阵测试 | `CPU-validated` + `Modeled` |
...
会作为静态规格单独展示。拓扑关系和配置带宽属于建模输入，不是硬件测量结果。
...
| `fakegpu nvidia-smi` | 查看设备、profile、runtime 状态、allocator 显存、进程和建模拓扑 |
