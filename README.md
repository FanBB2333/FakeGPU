...
| NCCL-style communication | Collective and point-to-point control flow, TCP socket payloads, deterministic direct-collective failure, controlled rank-process exit, communicator shrink/recovery, fixed-size `torchrun` worker-group restart, checkpoint resume, and partial gradient-accumulation recovery, topology-aware reporting, PyTorch-required NCCL 2.29 host symbols | Not a protocol-level NCCL/RDMA/NVLink model; no heartbeat-based membership resizing; device-communicator and signal/RMA operations are not simulated |
...
| Physical-host elastic DDP training-state recovery | One rank on each GPU; failure after one of two accumulation micro-steps | Both replacements restored AdamW moments, StepLR, rank-local RNG, optimizer step 1, and pending gradients; step 2 produced parameters `[0.983838, -0.014662]`; two repeated runs each reported 696 node-pair bytes and a 96-byte peak |
...
| `elastic_ddp_training_state` | Focused AdamW first/second moments, StepLR, rank-local RNG, optimizer-step, and partial gradient-accumulation recovery |
...
- The fixed-size elastic DDP checks rely on `torchrun` for worker supervision and rendezvous. Checkpoint recovery uses one atomic host-local file per worker and assumes the same world size after restart. Mid-accumulation recovery additionally requires each host to retain its saved global rank; rank reassignment needs shared, rank-addressable checkpoint storage. FakeGPU does not provide heartbeat-based detection or dynamic membership resizing. The direct-collective recovery path still requires survivors to call `ncclCommShrink` with an explicit exclusion list.
